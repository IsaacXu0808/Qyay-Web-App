<template>
    <div style="width: 600px; margin-left: auto; margin-right: auto;">
        ...
        <div style="color: red; font-size: smaller;">
            <div v-if="errState===0" style="color: rgb(132, 132, 132); font-weight: 370;">Please specify both date and time.</div>
            <div v-else-if="errState===1">Invalid length of event name.</div>
            ...
            <div v-else-if="errState===5">Please do not use a past date and time.</div>
            <div v-else-if="errState===6">Please specify both date and time.</div>
        </div>
        <div id="btn">
    ...
    export default{
        data(){
            return {
                ...
                eTime:"",
                eDate:"",
                errState: 0, // 1 name; 2 discription; 3 other； 5 invalid date & time; 6 missing time or date
                localTime: new Date(),
                compareTime:"def"
            }
        },
        ...
        methods:{
            createEvent(){
                if (this.eDate.length==0 || this.eTime.length==0) {
                    this.errState = 6;
                    return
                }
                const dateTimeString = (this.eDate + ' ' + this.eTime);
                const dateTime = new Date(dateTimeString); // set time
                const now = new Date();
                if (dateTime.getTime() < now.getTime()) this.errState = 5;
                else if (!this.logined) {this.errState = 4}
